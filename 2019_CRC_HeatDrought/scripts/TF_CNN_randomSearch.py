...
def main():
	...
	parser.add_argument('-save_detailed', help='T/F Save detailed model performance',
		type=str, default='f')
	parser.add_argument('-imp', help='row/col if should LOFO by row or column',
		type=str, default='')
	...
	if args.run.lower() in ['t', 'true']:
		...
		if args.params.lower() != 'default':
			...
			print('Parameter Search Coverate: \nMin: %i\nMean: %3f\nMax:%i' %
		...
		for n in range(args.n_reps):
			...
			model = make_cnn_model(learn_rate=args.learn_rate,
			...
			final_results = final_results.append({'ID': args.save, 
			...
			if (args.save_model.lower() in ['t', 'true'] or
				args.save_detailed.lower() in ['t', 'true']):
				save_name = args.save + '_model_' + str(n) + '.h5'
				model.save(save_name)

			if args.save_detailed.lower() in ['t', 'true']:

				#callbacks = [ActivationsVisualizationCallback(
				#	validation_data=(x_val, y_val),
				#	layers_name=["conv2d_layer"],
				#	output_dir='DND_out')]

				#model.fit(x_train, y_train, batch_size=2, epochs=2, callbacks=callbacks)

				#conv2d = model.get_layer(name='conv2d_layer').eval()
				conv2d = conv2d_layer.get_weights()[0]
